...
describe('save', () => {
  ...
  it('should not save user without an email', async () => {
    const user = new User({ password: faker.internet.password(), name: faker.person.firstName() })
    await expect(user.save()).rejects.toThrowError(/email/)
  })

  it('should not save user without a password', async () => {
    const user2 = new User({ email: faker.internet.email(), name: faker.person.firstName() })
  ...
  it('should not save password in a readable form', async () => {
    ...
    const user1 = new User({ email: faker.internet.email(), password: password, name: faker.person.firstName() })
    await user1.save()
    expect(user1.password).not.toBe(password)

    const user2 = new User({ email: faker.internet.email(), password: password, name: faker.person.firstName() })
    await user2.save()
...
describe('comparePassword', () => {
  it('should return true for valid password', async () => {
    const password = faker.internet.password()
    const user = new User({ email: faker.internet.email(), password: password, name: faker.person.firstName() })
    await user.save()
  ...
  it('should update password hash if password is updated', async () => {
    ...
    const user = new User({ email: faker.internet.email(), password: password1, name: faker.person.firstName() })
    const dbUser1 = await user.save()
